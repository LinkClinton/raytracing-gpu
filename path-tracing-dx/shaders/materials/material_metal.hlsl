...
metal_material convert_metal_material(material_gpu_buffer buffer, float2 value)
{
	metal_material material;

	material.eta = sample_texture(global_textures[buffer.eta], value);
	material.k = sample_texture(global_textures[buffer.k], value);
...
float3 evaluate_metal_material(metal_material material, float3 wo, float3 wi)
{
	if (wo.z == 0 || same_hemisphere(wo, wi) == false) return 0;

	return evaluate_conductor_microfacet_reflection(1, material.eta, material.k, material.roughness_u, material.roughness_v, wo, wi);
}
...
float pdf_metal_material(metal_material material, float3 wo, float3 wi)
{
...
scattering_sample sample_metal_material(metal_material material, float3 wo, float2 value)
{
...
float3 uniform_sample_one_emitter_with_metal_material(metal_material material, inout ray_payload payload)
{
...

	{
		...
		if (!is_black(emitter_sample.intensity) && emitter_sample.pdf > 0) {
			...
			float3 function_value = evaluate_metal_material(material, wo, wi);
			float function_pdf = pdf_metal_material(material, wo, wi);
	...
	{
		scattering_sample function_sample = sample_metal_material(material, wo, next_sample2d(payload.sampler));
...
bool sample_metal_material_and_lighting(inout ray_payload payload)
{
	metal_material material = convert_metal_material(global_materials[global_entities[payload.index].material], payload.interaction.uv);
	
	payload.value = uniform_sample_one_emitter_with_metal_material(material, payload);
