...
diffuse_material convert_diffuse_material(material_gpu_buffer buffer, float2 value)
{
...
float3 evaluate_diffuse_material(diffuse_material material, float3 wo, float3 wi)
{
...
float pdf_diffuse_material(diffuse_material material, float3 wo, float3 wi)
{
...
scattering_sample sample_diffuse_material(diffuse_material material, float3 wo, float2 value)
{
...
float3 uniform_sample_one_emitter_with_diffuse_material(diffuse_material material, inout ray_payload payload)
{
...

	{
		...
		if (!is_black(emitter_sample.intensity) && emitter_sample.pdf > 0) {
			...
			float3 function_value = evaluate_diffuse_material(material, wo, wi);
			float function_pdf = pdf_diffuse_material(material, wo, wi);
	...
	{
		scattering_sample function_sample = sample_diffuse_material(material, wo, next_sample2d(payload.sampler));
...
bool sample_diffuse_material_and_lighting(inout ray_payload payload) 
{
	diffuse_material material = convert_diffuse_material(global_materials[global_entities[payload.index].material], payload.interaction.uv);

	payload.value = uniform_sample_one_emitter_with_diffuse_material(material, payload);
