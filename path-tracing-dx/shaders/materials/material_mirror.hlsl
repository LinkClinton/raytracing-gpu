...
float3 evaluate_mirror_material(material_gpu_buffer material, float3 wo, float3 wi, scattering_type include = scattering_all)
{
...
float pdf_mirror_material(material_gpu_buffer material, float3 wo, float3 wi, scattering_type include = scattering_all)
{
	if (has(include, scattering_type(scattering_reflection | scattering_specular)))
...
scattering_sample sample_mirror_material(material_gpu_buffer material, float3 wo, float2 value, scattering_type include = scattering_all)
{
